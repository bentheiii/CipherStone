    ...
    public class Compression
    {
        ...
        public void Simple()
        {
            ...
            foreach (string s in strings)
            {
                ...
                var cipher = plainbytes.Compress();
                var deciphered = cipher.Decompress();
                var dectext = Encoding.UTF8.GetString(deciphered);
                //Assert.IsTrue(dectext.SequenceEqual(s),(i++).ToString());
                int j = 0;
                foreach (var tuple in s.Zip(dectext))
                {
                    var orig = tuple.Item1;
                    var dec = tuple.Item2;

                    Assert.AreEqual(orig, dec, $"string {i}, char {j++}");
                }
                i++;
            }
        }
