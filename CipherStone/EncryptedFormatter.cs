...
namespace CipherStone
{
    ...
    public class EncryptedFormatterV3<T> : IFormatter<T>
    {
        ...
        private readonly (int minInc, int maxExc) _paddingRange;
        public EncryptedFormatterV3(IFormatter<T> inner, byte[] key, SecureEncryptionV3.EncryptionOptions options, (int, int)? paddingRange = null)
        {
            _inner = inner;
            _key = key;
            _options = options;
        ...
        {
            var padding = _getPadding();
            using (var s = SecureEncryptionV3.EncryptStream(sink, _key, _options, padding))
            {
        ...
        public int SerializeSize(T o)
        {
            ...
            if (i == -1)
                return -1;
            return SecureEncryptionV3.EncyptSize(i, _options, _paddingRange.minInc);
        }
